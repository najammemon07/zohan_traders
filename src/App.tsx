        ...
        <Container maxW="1200px" py="16px">
          <Flex align="center" justify="space-between">
            <a href="#home" aria-label="Zohan Traders home">
              <HStack gap="3"><Box className="brand-mark">ZT</Box><Box display={{ base: 'none', sm: 'block' }}><Text fontWeight="800" letterSpacing="2px" fontSize="14px">ZOHAN TRADERS</Text><Text color="#a6a6a6" fontSize="9px" letterSpacing="2.5px">REFINED SALT</Text></Box></HStack>
            </a>
            <HStack gap="8" display={{ base: 'none', md: 'flex' }}>{navItems.map(([label, href]) => <a className="nav-link" key={label} href={href}>{label}</a>)}</HStack>
        ...
        <Container maxW="1200px" py={{ base: '55px', md: '65px' }}>
          <SimpleGrid columns={{ base: 1, sm: 2, md: 4 }} gap={{ base: '10', md: '8' }}>
            <VStack align="start" gap="4">
              <HStack gap="3"><Box className="brand-mark">ZT</Box><Box><Text fontWeight="800" letterSpacing="2px" fontSize="14px">ZOHAN TRADERS</Text><Text color="#888" fontSize="9px" letterSpacing="2.5px">REFINED SALT</Text></Box></HStack>
              <Text color="#777" fontSize="13px" lineHeight="1.8" maxW="270px">Quality refined iodized salt, carefully packed for homes, retailers and growing businesses.</Text>
            </VStack>
            <VStack align="start" gap="3"><Text className="footer-heading">QUICK LINKS</Text>{navItems.map(([label, href]) => <a className="footer-link" key={label} href={href}>{label}</a>)}</VStack>
            <VStack align="start" gap="3"><Text className="footer-heading">OUR BRAND</Text><Text color="#d4af37" fontWeight="700">ZT ZAIQO</Text><Text color="#777" fontSize="13px">Refined Iodized Salt</Text><Text color="#777" fontSize="13px">600g • 700g • 800g</Text></VStack>
            <VStack align="start" gap="3"><Text className="footer-heading">CONTACT</Text><Text color="#999" fontSize="13px">📍 Hyderabad, Site Area, Pakistan</Text><a className="footer-link" href="tel:+923133976670">0313 3976670</a><a className="footer-link" href="tel:+923097431271">0309 7431271</a></VStack>
          </SimpleGrid>
          <Box className="footer-bottom"><Text color="#666" fontSize="12px">© 2026 Zohan Traders. All rights reserved.</Text><Text color="#555" fontSize="11px" letterSpacing="1.5px">PURE • CLEAN • RELIABLE</Text></Box>
        </Container>
      </Box>
    </Box>
  );
